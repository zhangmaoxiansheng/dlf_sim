...
from ply_io import read_points3D_text,storePly
...
def bin2txt(dataset_path):
    model = os.path.join(dataset_path)
    if os.path.exists(model):
        cameras, images, points3D = read_model(path=model, ext='.bin')
        output_path_ = model
        write_model(cameras, images, points3D, path=output_path_, ext='.txt')

if __name__ == "__main__":
    #main()
    #os.makedirs('trash_colmap_fail',exist_ok=True)
